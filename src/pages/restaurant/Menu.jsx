    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">Menu Inventory</h1>
                    <p className="text-slate-500 font-medium italic mt-1">Manage your dishes, pricing, and availability</p>
                </div>
                <Button onClick={() => { resetForm(); setIsAddModalOpen(true); }} className="gap-2 px-6">
                    <Plus size={20} />
                    Add New Dish
                </Button>
            </div>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                    <div className="relative flex-1 group w-full">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-slate-50 border-transparent focus:bg-white focus:border-orange-200 focus:ring-4 focus:ring-orange-500/5 transition-all text-sm font-bold"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                        />
                    </div>
                    <Button variant="outline" className="gap-2 sm:w-auto w-full h-[52px]">
                        <Filter size={18} />
                        Filters
                    </Button>
                </div>
            ...
            {loading ? (
                ...
                <>
                    <Table headers={['Dish', 'Category', 'Price', 'Type', 'Status', 'Actions']}>
                        {menu.map((item) => (
                            <tr key={item._id} className="hover:bg-slate-50/50 transition-colors group">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-4">
                                        {item.imageUrl ? (
                                            <img src={item.imageUrl} alt={item.name} className="w-12 h-12 rounded-xl object-cover border border-slate-100 shadow-sm" />
                                        ) : (
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-400 font-medium max-w-[200px] truncate">{item.description}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button
                                            ...
                                            className="p-2 hover:bg-white hover:text-orange-500 hover:shadow-md rounded-xl transition-all"
                                        >
                                            <Edit2 size={18} />
                                        </button>
                                        <button
                                            onClick={() => { setSelectedItem(item); setIsDeleteModalOpen(true); }}
                                            className="p-2 hover:bg-white hover:text-red-500 hover:shadow-md rounded-xl transition-all"
                                        >
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                ...
                <EmptyState title="No Items Found" message="Try searching for something else or add a new dish." />
            )}
            ...
            {(isAddModalOpen || isEditModalOpen) && (
                <Modal
                    ...
                    footer={(
                        <>
                            ...
                            <Button onClick={isAddModalOpen ? handleAddSubmit : handleEditSubmit}>
                ...
                >
                    <div className="space-y-6">
                        <div className="flex flex-col items-center justify-center p-8 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 group relative overflow-hidden">
                            {formData.imageUrl ? (
                                <>
                        ...
                        <div className="flex items-center gap-8 px-2">
                            <label className="flex items-center gap-2 cursor-pointer group">
                                <input
                                    type="checkbox"
                                    className="w-5 h-5 rounded-lg border-slate-300 text-orange-500 focus:ring-orange-500/20"
                                    checked={formData.isVeg}
                                    onChange={(e) => setFormData({ ...formData, isVeg: e.target.checked })}
                                />
                                <span className="text-sm font-bold text-slate-600 group-hover:text-slate-900 transition-colors">Vegetarian</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer group">
                                <input
                                    type="checkbox"
                                ...
                                />
                                <span className="text-sm font-bold text-slate-600 group-hover:text-slate-900 transition-colors">Available</span>
                            </label>
                        </div>
                    </div>
                </Modal>
            ...
            <Modal
                ...
                footer={(
                    <>
                        <Button variant="ghost" onClick={() => setIsDeleteModalOpen(false)}>Keep Item</Button>
                        <Button variant="danger" onClick={handleDeleteSubmit}>Yes, Delete</Button>
                    </>
                )}
            >
                <div className="text-center py-4">
                    ...
                    <p className="text-slate-500 font-medium">
                        Are you sure you want to delete <span className="font-black text-slate-900">"{selectedItem?.name}"</span>?
