...
const DashboardLayout = ({ role }) => {
    ...
    const handleSearch = (e) => {
        if (e.key === 'Enter') {
            toast.success(`Search functionality ready: Finding "${searchQuery}"...`);
            // In a real app, navigate to search results: navigate(\`/admin/search?q=\${searchQuery}\`);
        }
    };
    ...
    // GLASSMORPHISM THEME CONSTANTS - PROFESSIONAL
    const sidebarBg = isSuperAdmin
        ? 'bg-white/5 backdrop-blur-xl border-white/10'
        : 'bg-slate-900 border-slate-800';

    // Professional Gradient for Super Admin (Deep Slate/Blue)
    const mainBg = isSuperAdmin
        ? 'bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950'
        : 'bg-slate-50';

    const contentBg = isSuperAdmin ? 'bg-transparent' : 'bg-[#f8fbff]';

    const brandColor = isSuperAdmin ? 'text-white drop-shadow-md' : 'text-orange-500';

    // Glass active item 
    const activeItemBg = isSuperAdmin
        ? 'bg-white/10 backdrop-blur-md shadow-lg border border-white/5'
        : 'bg-orange-500';

    const hoverBg = isSuperAdmin ? 'hover:bg-white/5' : 'hover:bg-slate-800/50';

    // Glass header
    const headerBg = isSuperAdmin
        ? 'bg-white/5 backdrop-blur-xl border-white/10 shadow-sm'
        : 'bg-white border-slate-200';

    const headerText = isSuperAdmin ? 'text-white' : 'text-slate-900';
    const subText = isSuperAdmin ? 'text-slate-300' : 'text-slate-400';
            ...
            >
                <div className={cn("p-6 flex items-center justify-between mb-4 relative z-10")}>
                    {isSidebarOpen ? (
                        ...
                        <div className={cn(
                            "w-12 h-12 rounded-xl flex items-center justify-center font-bold shadow-lg mx-auto",
                            isSuperAdmin ? "bg-white/10 border border-white/10 backdrop-blur-md text-white" : "bg-orange-500 text-white"
                        )}>
                            {isSuperAdmin ? <Shield size={24} /> : 'Z'}
                        </div>
                    )}
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <header className={cn("h-20 flex items-center justify-between px-8 z-10 transition-all", headerBg)}>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className={cn(
                            "hidden lg:flex items-center px-5 py-3 rounded-full border transition-all w-80",
                            isSuperAdmin
                                ? "bg-white/5 border-white/10 text-white focus-within:bg-white/10 focus-within:border-white/20 placeholder-white/40"
                                : "bg-slate-100 border-transparent text-slate-500 focus-within:border-orange-200 focus-within:bg-white"
                        )}>
                            <Search size={22} className={isSuperAdmin ? "text-white/50" : "text-slate-400"} />
                            <input
                                type="text"
                                placeholder="Search..."
                                value={searchQuery}
                                ...
                                className={cn(
                                    "bg-transparent border-none focus:ring-0 text-base ml-2 w-full font-medium",
                                    isSuperAdmin ? "placeholder:text-white/30 text-white" : "placeholder:text-slate-500"
                                )}
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-3 relative">
                            <button
                                ...
                                <Bell size={20} />
                                <span className={cn("absolute top-2 right-2.5 w-2 h-2 rounded-full border-2",
                                    isSuperAdmin ? "bg-blue-500 border-transparent" : "bg-orange-500 border-white"
                                )}></span>
                            </button>

                            {/* Notification Dropdown */}
                            {isNotificationsOpen && (
                                <div className="absolute top-14 right-0 w-80 bg-[#0f172a]/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 z-50">
                                            ...
                                            notifications.map((notif, idx) => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className="p-4 hover:bg-white/5 border-b border-white/5 last:border-0 cursor-pointer flex gap-3 transition-colors"
                                                    >
                                                        <div className="mt-1"><Icon size={16} className={notif.color || 'text-white'} /></div>
                                                        <div>
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className={cn(
                                        "h-12 w-12 rounded-full flex items-center justify-center font-bold shadow-md transition-all hover:scale-105 active:scale-95 cursor-pointer",
                                        isSuperAdmin
                                            ? "bg-gradient-to-tr from-blue-500/20 to-blue-600/20 text-white border border-white/20 backdrop-blur-md hover:bg-white/10"
                                ...
                                {/* Profile Dropdown */}
                                {isProfileOpen && (
                                    <div className="absolute top-16 right-0 w-80 bg-[#0f172a]/95 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 z-50">
